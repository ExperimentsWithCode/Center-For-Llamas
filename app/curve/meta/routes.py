...
def index():
    ...
    fig = px.bar(df_head,
                    ...
                    title=f"Power Difference: Leader Board, Round: -{this_round}, Date: {df_head.iloc[0]['period_end_date'].date()}",
                    # line_shape='hvh'
                    height=600
                    )
    ...
    fig = px.bar(df_tail,
                        ...
                        # color=df_formated_shorts['name'],
                    title=f"Power Difference: Leader Board, Round: -{this_round}, Date: {df_tail.iloc[0]['period_end_date'].date()}",
                    # line_shape='hvh'
                    height=600
                    )
    fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
