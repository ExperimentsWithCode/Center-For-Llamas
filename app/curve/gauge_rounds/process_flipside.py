...
def process_and_save():
    ...
    df_book = generate_aggregation(df_curve_locker_history, df_gauge_votes_formatted)
    ...
    write_dataframe_csv(filename_curve_gauge_rounds_by_user, df_all_by_user, 'processed')

    df_all_by_gauge = concat_all(df_combo_by_gauge_list,  ['this_period', 'total_vote_power' ])
    write_dataframe_csv(filename_curve_gauge_rounds_by_aggregate, df_all_by_gauge, 'processed')
    ...
    return {
        'df_gauge_rounds_all_by_user': df_all_by_user,
        'df_gauge_rounds_aggregate': df_all_by_gauge
    }
