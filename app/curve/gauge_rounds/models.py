...
def format_df(df):
    ...
    if 'final_lock_time' in key_list:
        df['final_lock_time']       = pd.to_datetime(df['final_lock_time']).dt.date
    ...
    if 'vote_percent' in key_list:
        df['vote_percent']          = df['vote_percent'].astype(float)
    if 'vote_power' in key_list:
        df['vote_power']            = df['vote_power'].astype(float)
    if 'vote_utilization' in key_list:
        df['vote_utilization']      = df['vote_utilization'].astype(float)
    return df
...
df_all_by_user = get_df(filename_curve_gauge_rounds_by_user)
df_all_by_gauge = get_df(filename_curve_gauge_rounds_by_aggregate) 
