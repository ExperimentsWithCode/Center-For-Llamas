...
#     # Filter Data
#     selection = [delegator]
#     mask = df_vote_choice.de.apply(lambda x: any(item for item in selection if item in x))
...
#     local_df_locker_user_epoch = df_convex_locker_agg_user_epoch[df_convex_locker_agg_user_epoch['delegate'] == delegate]
...
#         title='Convex Delegator Voter Profile',
...
def delegator(delegator):
    ...
    local_known_as = local_df_locker_user_epoch.iloc[0]['known_as']
